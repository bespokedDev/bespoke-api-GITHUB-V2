...
// Acceso: Solo admin
router.post('/', verifyToken, verifyRole('admin'), categoryNotificationCtrl.create);

// GET /api/category-notifications - Lista todas las categorías de notificación
// Acceso: Solo admin
router.get('/', verifyToken, verifyRole('admin'), categoryNotificationCtrl.list);

// GET /api/category-notifications/:id - Obtiene una categoría de notificación por su ID
// Acceso: Solo admin
router.get('/:id', verifyToken, verifyRole('admin'), categoryNotificationCtrl.getById);

// PUT /api/category-notifications/:id - Actualiza una categoría de notificación por su ID
// Acceso: Solo admin
router.put('/:id', verifyToken, verifyRole('admin'), categoryNotificationCtrl.update);

// PATCH /api/category-notifications/:id/anular - Anula una categoría de notificación
// Acceso: Solo admin
router.patch('/:id/anular', verifyToken, verifyRole('admin'), categoryNotificationCtrl.anular);

// PATCH /api/category-notifications/:id/activate - Activa una categoría de notificación
// Acceso: Solo admin
router.patch('/:id/activate', verifyToken, verifyRole('admin'), categoryNotificationCtrl.activate);
