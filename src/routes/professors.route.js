...
// Acceso: Solo admin
router.post('/', verifyToken, verifyRole('admin'), createProfessorValidation, professorCtrl.create);

// GET /api/professors - Listar todos los profesores
// Acceso: Solo admin
router.get('/', verifyToken, verifyRole('admin'), professorCtrl.list);
...
// Acceso: Admin y professor
router.put('/:id', verifyToken, verifyRole('admin', 'professor'), professorCtrl.update);

// PATCH /api/professors/:id/deactivate - Desactivar profesor
// Acceso: Solo admin
router.patch('/:id/deactivate', verifyToken, verifyRole('admin'), professorCtrl.deactivate);

// PATCH /api/professors/:id/activate - Activar profesor
// Acceso: Solo admin
router.patch('/:id/activate', verifyToken, verifyRole('admin'), professorCtrl.activate);

// PATCH /api/professors/uniformize-payment-ids - Uniformizar IDs de pago
// Acceso: Solo admin
router.patch('/uniformize-payment-ids', verifyToken, verifyRole('admin'), professorCtrl.uniformizePaymentIds);

// GET /api/professors/debug/payment-data - Debug de datos de pago
// Acceso: Solo admin
router.get('/debug/payment-data', verifyToken, verifyRole('admin'), professorCtrl.logPaymentData);
